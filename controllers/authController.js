...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (!email || !password || !{ active: false }) {
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // Grant access to prottected route
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError(
        'There was an error saving your password. Please try again later.',
        500
      )
    );
  }
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // If the token not expired and there is user, then set the new password
  if (!user) {
  ...
  // Update passwordChangeAt propertie for the user
...
exports.updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (!(await user.correctPassword(req.body.passwordCurrent, user.password))) {
    return next(new AppError('Your password does not match!', 401));
  }
